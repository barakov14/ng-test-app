...
function isSignal<T>(loadingIndicator: WritableSignal<T> | BehaviorSubject<T>): loadingIndicator is WritableSignal<T> {
  return 'set' in loadingIndicator;
}

// Operator for managing loading state
export function withLoading<T>(loadingIndicator: WritableSignal<boolean> | BehaviorSubject<boolean>): OperatorFunction<T, T> {
